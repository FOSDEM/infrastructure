...
/usr/local/bin/bmd-streamer -f /usr/lib/firmware -k 1000 -S hdmi -F 0 2>> /mnt/ssd/bmd-streamer.log | ffmpeg -i - -c copy -f mpegts '{{ bmd_streamer_destination }}&{{ bmd_streamer_ffmpeg_options }}'
