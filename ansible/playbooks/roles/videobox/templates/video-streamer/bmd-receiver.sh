...
/usr/local/bin/bmd-streamer -f /usr/lib/firmware -k 4000 -S hdmi -F 0 2>> ${LOGFILE} | ffmpeg -i - -c copy -f tee -map 0:v -map 0:a '[f=mpegts]'"udp://127.0.0.1:4000?${STREAM_OPTIONS}"'|[f=mpegts]'"udp://127.0.0.1:4001?${STREAM_OPTIONS}"
