...
$data = http_build_query(array(
    'db' => 'ebur',
    'q' => 'SELECT mean(S) as S, mean(M) as M FROM "ebur2" WHERE time >= ' . $lastts . ' and time > now() - 5m and time <= now() GROUP BY time(1s), chan fill(null)',
    'epoch' => 'ms'
));

$ch = curl_init('http://127.0.0.1:8086/query?'.$data);
