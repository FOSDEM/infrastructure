...
if ! $ts ls >/dev/null 2>/dev/null; then
	ts="systemd-run --scope -p AllowedCPUs=2"
fi

LD_PRELOAD=/usr/lib/cursor_disable.so $ts ffmpeg -y -v verbose -nostdin -init_hw_device vaapi=intel:/dev/dri/renderD128 -hwaccel vaapi -hwaccel_output_format vaapi -hwaccel_device intel -filter_hw_device intel  \
